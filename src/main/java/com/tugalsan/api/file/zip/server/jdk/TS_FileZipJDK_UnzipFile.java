...
public class TS_FileZipJDK_UnzipFile {
    ...
    public static TGS_UnionExcuseVoid unzip(Path fileZip, Path destPath) {
        ...
        try (var zis = new ZipInputStream(Files.newInputStream(fileZip))) {
            ...
            while ((zipEntry = zis.getNextEntry()) != null) {
                ...
                if (u_newFile.isExcuse()) {
                    return TGS_UnionExcuseVoid.ofExcuse(u_newFile.excuse());
                }
                var newFile = u_newFile.value();
                if (zipEntry.isDirectory()) {
                    if (!newFile.toFile().mkdirs()) {
                        return TGS_UnionExcuseVoid.ofExcuse(TS_FileZipJDK_UnzipFile.class.getSimpleName(), "unzip", "Failed to create directory " + newFile);
                    }
                } else {
                    var parent = newFile.toFile().getParentFile();
                    if (!parent.isDirectory() && !parent.mkdirs()) {
                        return TGS_UnionExcuseVoid.ofExcuse(TS_FileZipJDK_UnzipFile.class.getSimpleName(), "unzip", "Failed to create directory " + parent);
                    }
     ...
     */
    public static TGS_UnionExcuse<Path> newFile(File destinationDir, ZipEntry zipEntry) {
        try {
            ...
            if (!destFilePath.startsWith(destDirPath + File.separator)) {
                return TGS_UnionExcuse.ofExcuse(TS_FileZipJDK_UnzipFile.class.getSimpleName(), "newFile", "Entry is outside of the target dir: " + zipEntry.getName());
            }
            return TGS_UnionExcuse.of(destFile.toPath());
        } catch (IOException ex) {
